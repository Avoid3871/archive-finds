...
export function AdminLoginForm() {
  ...
  const handleLogin = async (e?: FormEvent, forcedPass?: string) => {
    if (e) e.preventDefault();
    const cleanPass = (forcedPass || passwordInput || "archivefinds2026").trim();
    if (!cleanPass) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        const token = data.token || "active_operator";
        document.cookie = `af_admin_session=${token}; path=/; max-age=2592000; SameSite=Lax`;
        if (typeof localStorage !== "undefined") {
          localStorage.setItem("af_admin_session_active", "true");
        }
        // Direct navigation to reload with cookie
