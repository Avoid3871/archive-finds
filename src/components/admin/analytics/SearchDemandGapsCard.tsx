...
export function SearchDemandGapsCard({
  ...
  const handleRunTestSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTestSuccessNotice(
        isDe
          ? `✓ Suchanfrage „${testQuery.trim()}“ live geloggt!`
          : `✓ Search query "${testQuery.trim()}" logged live!`
      );
      setTestQuery("");
  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 space-y-6 shadow-xl relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-800 pb-5">
        <div>
          ...
          <p className="text-xs font-mono text-neutral-400 mt-1">
            {isDe
              ? "Erfasst echte Suchanfragen von Besuchern und gleicht sie in Echtzeit mit deinem Live-Katalog ab."
              : "Logs authentic visitor search terms and cross-references them against your live catalog."}
          </p>
        </div>
      ...
      {/* Interactive Quick-Test Search Logger */}
      <div className="p-3.5 bg-neutral-950 rounded-xl border border-neutral-800/90 space-y-2">
        <div className="flex items-center justify-between text-xs font-mono">
          <span className="text-neutral-300 font-bold flex items-center gap-1.5">
            ...
            <span>{isDe ? "Live-Pipeline testen (Suchbegriff erfassen):" : "Test Live Search Pipeline:"}</span>
          </span>
          {testSuccessNotice && (
        ...
        <form onSubmit={handleRunTestSearch} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={
                isDe
                  ? "Suchbegriff eingeben (z. B. 'Rick Owens Dunks' oder 'Balenciaga Cargo')..."
                  : "Type a search query to log (e.g. 'Rick Owens Dunks')..."
              }
      ...
      {/* Demand Gaps Content: Real Data Table OR Clean Empty State */}
      {gaps.length > 0 ? (
        ...
        <div className="p-8 text-center bg-neutral-950/60 rounded-xl border border-neutral-800/80 space-y-3">
          ...
          <div className="space-y-1">
            <h3 className="text-xs font-mono font-bold uppercase text-white tracking-wider">
              {isDe ? "Bereit für organische Suchanfragen" : "Listening for Organic Searches"}
            </h3>
            <p className="text-xs text-neutral-400 font-sans max-w-md mx-auto">
              {isDe
                ? "Sobald Besucher in der Suchleiste deines Shops nach Designern oder Pieces suchen, werden die Begriffe hier in Echtzeit geloggt und mit deinem Katalog abgeglichen."
                : "As soon as visitors search for pieces in your shop search bar, their queries will appear here in real-time."}
            </p>
          </div>
        </div>
      )}
